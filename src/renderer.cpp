...
void main() {
    frag_color = vec4(texture(tex_color, uv).rgb, 1.0);
}
)";
...
void Renderer::upload( const float* rgb ) {
  glBindTexture( GL_TEXTURE_2D, texture_color );
  glTexSubImage2D( GL_TEXTURE_2D, 0, 0, 0, width, height, GL_RGB, GL_FLOAT, rgb );
  glBindTexture( GL_TEXTURE_2D, 0 );
}
...
void Renderer::draw() {
  ...
  glBindTexture( GL_TEXTURE_2D, texture_color );
  glUniform1i( glGetUniformLocation( shader_program, "tex_color" ), 0 );
  glBindVertexArray( vao );
