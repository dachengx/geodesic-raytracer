...
int main( void ) {
  ...
  // A second VBO containing one RGB colour per vertex.
  float colours[] = {
    1.0f, 0.0f, 0.0f, // red
  ...
  // Upload colour data to a separate VBO.
  GLuint colours_vbo = 0;
  glGenBuffers( 1, &colours_vbo );
  glBindBuffer( GL_ARRAY_BUFFER, colours_vbo );
  glBufferData( GL_ARRAY_BUFFER, 9 * sizeof( float ), colours, GL_STATIC_DRAW );

  // VAO remembers which VBO is bound to which attribute index.
  GLuint vao = 0;
  glGenVertexArrays( 1, &vao );
  glBindVertexArray( vao );
  glBindBuffer( GL_ARRAY_BUFFER, points_vbo );
  glVertexAttribPointer( 0, 3, GL_FLOAT, GL_FALSE, 0, NULL ); // attribute 0 = position
  glBindBuffer( GL_ARRAY_BUFFER, colours_vbo );
  glVertexAttribPointer( 1, 3, GL_FLOAT, GL_FALSE, 0, NULL ); // attribute 1 = colour
